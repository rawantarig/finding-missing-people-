...
showVolunteerSection(‘all-reports’);
}
...
// دوال التسجيل مع التحقق من كلمة المرور
function registerUser() {
    const name = document.getElementById('user-name').value;
    const email = document.getElementById('user-email').value;
    const phone = document.getElementById('user-phone').value;
...
function loginUser() {
    ...
    if (storedUser) {
        ...
        alert('لا يوجد حساب بهذه البيانات');
    }
}

// دوال مماثلة للمتطوعين والشرطة
function registerVolunteer() {
    const name = document.getElementById('volunteer-name').value;
    const email = document.getElementById('volunteer-email').value;
    const password = document.getElementById('volunteer-password').value;
...
function registerPolice() {
    ...
    const confirmPassword = document.getElementById('police-confirm-password').value;
...
// دوال تسجيل الدخول للمتطوعين والشرطة
function loginVolunteer() {
    const email = document.getElementById('volunteer-login-email').value;
    const password = document.getElementById('volunteer-login-password').value;
    ...
    if (storedVolunteer) {
        const volunteerData = JSON.parse(storedVolunteer);
        if (volunteerData.email === email && volunteerData.password === password) {
...
function loginPolice() {
    ...
    if (storedPolice) {
        const policeData = JSON.parse(storedPolice);
        if (policeData.email === email && policeData.password === password) {
            currentPolice = policeData;
            showPoliceDashboard();
        } else {
            alert('البريد الإلكتروني أو كلمة المرور غير صحيحة');
        }
...
// دالة لعرض لوحة تحكم المستخدم
function showUserDashboard() {
    document.getElementById('main-page').classList.add('hidden');
    document.getElementById('user-dashboard').classList.remove('hidden');
    
    // تعبئة بيانات المستخدم
    document.getElementById('user-welcome').textContent = `مرحباً ${currentUser.name}`;
    document.getElementById('user-phone-display').textContent = currentUser.phone;
    document.getElementById('user-email-display').textContent = currentUser.email;
    
    // تعبئة رقم الهاتف في حقل إضافة البلاغ تلقائياً
    document.getElementById('reporter-phone').value = currentUser.phone;
    
    // إظهار قسم إضافة البلاغ افتراضياً
    showUserSection('add-report');
}
...
function showPoliceDashboard() {
    ...
    // إظهار البلاغات الجديدة افتراضياً
    showPoliceSection('new-reports');
}
...
function showVolunteerDashboard() {
    ...
    // إظهار جميع البلاغات افتراضياً
    showVolunteerSection('all-reports');
}

// دوال تحميل بيانات الشرطة
function loadPoliceData() {
    loadReportsByStatus('new-reports-list', 'جديد');
    loadReportsByStatus('in-progress-reports-list', 'قيد العمل');
    loadReportsByStatus('resolved-reports-list', 'تم الحل');
...
function createPoliceReportItem(report) {
    ...
    // زر الاتصال بالمبلغ مع عرض الاسم والرقم
    actions += `<button class="btn btn-success" onclick="contactReporter('${report.reporterPhone}', '${report.reporterName}')">
        📞 الاتصال بـ ${report.reporterName} - ${report.reporterPhone}
    </button>`;
    ...
    reportItem.innerHTML = `
        ...
        <div class="contact-info">
            ...
            <p><strong>رقم الهاتف:</strong> ${report.reporterPhone}</p>
            ${report.additionalContact ? `<p><strong>رقم إضافي:</strong> ${report.additionalContact}</p>` : ''}
            ${report.userPhone2 ? `<p><strong>رقم هاتف إضافي:</strong> ${report.userPhone2}</p>` : ''}
        </div>
        ${report.photo ? `
        <div class="photo-preview-container">
            <h4>صورة المفقود:</h4>
        ...
        ` : ''}
        <p><strong>الحالة:</strong> <span class="status-${getStatusClass(report.status)}">${report.status}</span></p>
        <p><strong>التاريخ:</strong> ${report.date}</p>
        <div class="action-buttons">${actions}</div>
    `;
...
function contactReporter(phone, name) {
    showCustomNotification(`جاري الاتصال بـ ${name} على الرقم: ${phone}`, 'info');
    // يمكن إضافة ربط بالهاتف هنا
    window.open(`tel:${phone}`);
}
// دوال العرض المصححة - استخدام style.display مباشرة
function showRegister(platform) {
    console.log('فتح تسجيل:', platform);
    
    // إخفاء جميع النماذج باستخدام style.display
    const allForms = document.querySelectorAll('.form-container');
    allForms.forEach(form => {
        form.style.display = 'none';
    });
    
    // إخفاء منصات الاختيار
    const allPlatforms = document.querySelectorAll('.platform-box');
    allPlatforms.forEach(box => {
    ...
    if (registerForm) {
        ...
        console.log('تم عرض نموذج التسجيل');
    }
}

function showLogin(platform) {
    console.log('فتح دخول:', platform);
    ...
    if (loginForm) {
        ...
        console.log('تم عرض نموذج الدخول');
    }
}
...
function showMainPage() {
    ...
    document.getElementById('police-dashboard').style.display = 'none';
